...
name=${name:-payout-notifiacation}  # provide the action name to be invoked when event comes
workflowInvoker=${workflowInvoker:-workflow-invoker}
...
$WSK_CLI -i --apihost "$openwhiskApiHost" rule update "$TRIGGER-polkadot-payout" $TRIGGER $workflowInvoker --auth "$openwhiskApiKey"
