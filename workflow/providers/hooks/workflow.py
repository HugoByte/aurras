...
def struct_generator():
    ...
    global impl_get_task_trait
    global task_struct
    global impl_stucture
    global run
    global new_method
    global setter
    global task_store
...
"""
    ...
    task_struct += impl_get_task_trait
    task_struct += impl_task_trait

    task_struct += impl_stucture.replace("'", "")
    return
...
def create_sturct(task_name: str, args: list, type: str, kind):
    ...
    for prop in action_props['action']:
        if convert_to_pascalcase(prop['name']) == task_name:
            action_prop += f"""
#[AuthKey="{prop['auth_token']}"]
#[ApiHost="{prop['api_host']}"]
#[Insecure="{prop['insecure'].lower()}"]
...
"""
    ...
    task_struct = task_struct.replace("\\", "")
    task_struct = task_struct.rjust(len(task_struct))
...
def create_main_input_struct():
    ...
    global task_struct
    global new_method
    global generic_input_sturct_filed
...
"""
    ...
    implement_new_and_setter(tasks, task_store_copy, setter, dependency)
    return


def implement_new_and_setter(tasks, task_store_copy, setter, dependency):
    global task_struct
...
"""
        ...
        if task_name in setter.keys():
            ...
            dep_task_out = convert_to_pascalcase(" ".join(dependency[task_name].keys()))
...
def create_main_function(task) -> str:
    global task_store
    global task_store_copy
    global dependency_matrix
    global task_struct
...
class WorkFlow(BaseHook):
    ...
    action_properties : Any = Field(...,title="action_properties",description="Porperties for the actions defined")
    ...
    def exec(self):
        ...
        action_props = self.action_properties
        struct_generator()
        create_main_input_struct()
        main_file_input = create_main_function(tasks)
        output(main_file_input)

        
        return

def output(main_file):
    global cargo_file,common_rs_file,traits_file,task_struct
    output_path = "../../"
    ...
    rustfile.writelines(task_struct)
    rustfile.close()
